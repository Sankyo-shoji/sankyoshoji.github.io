...
function subForm() {
  ...
  for (let j = 1; j < i; j++) {
    ...
    // 修理内容について
    sympromps_buff[j] = clone_element[j].querySelector(
      'input[name="symptoms"]:checked'
    ).value;
    if (sympromps_buff[j] == 0) {
      sympromps[j] = clone_element[j].querySelector(
        'textarea[name="symptoms"]'
      ).value;
    } else {
      sympromps[j] = "納品のみ";
      sympromps_name[j] = clone_element[j].querySelector(
        'input[name="symptoms_name"]'
      ).value;
      sympromps_count[j] = clone_element[j].querySelector(
        'input[name="symptoms_number"]'
      ).value;
    }
